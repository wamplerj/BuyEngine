...
namespace BuyEngine.Data.Sql
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<Product> GetAsync(string sku)
        {
            ...
            var product = await QueryProduct(ProductQueries.GetBySku, new { @sku = sku });
        ...
        public async Task<PagedList<Product>> GetAllBySupplierAsync(int supplierId, int pageSize, int page)
        {
            _logger.Debug($"Querying for Product All Products for Supplier: {supplierId}, Page: {page}, PageSize: {pageSize}");
        ...
        public async Task<PagedList<Product>> GetAllByBrandAsync(int brandId, int pageSize, int page)
        {
            _logger.Debug($"Querying for Product All Products for Brand: {brandId}, Page: {page}, PageSize: {pageSize}");
        ...
        private async Task<PagedList<Product>> QueryProducts(string sql, object parameters, int pageSize, int page)
        {
            ...
            var products = result.Read(_mapProduct, splitOn: "BrandId,SupplierId");
        ...
        private async Task<Product> QueryProduct(string sql, object parameters)
        {
            ...
            var product = (await connection.
                    QueryAsync<Product, Brand, Supplier, Product>(sql, _mapProduct, parameters, splitOn: "BrandId,SupplierId"))
                .FirstOrDefault();
