...
public class SupplierService : ISupplierService
{
    ...
    public async Task<Supplier> GetAsync(Guid supplierId) => await _supplierRepository.GetAsync(supplierId);

    public async Task<IPagedList<Supplier>> GetAllAsync(int pageSize = CatalogConfiguration.DefaultRecordsPerPage, int page = 0) =>
        await _supplierRepository.GetAllAsync(pageSize, page);
    ...
    public async Task<ValidationResult> ValidateAsync(Supplier supplier) => await _validator.ValidateAsync(supplier);
}

public interface ISupplierService
{
    Task<Supplier> GetAsync(Guid supplierId);
    Task<IPagedList<Supplier>> GetAllAsync(int pageSize = 25, int page = 0);
